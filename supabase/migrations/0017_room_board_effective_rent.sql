-- 0017_room_board_effective_rent.sql
-- v_room_board.monthly_rent was always r.monthly_rent -- the room's own
-- (often stale/seeded) column -- even for an occupied room actually billed at
-- its contract's rent, and even for a vacant room that should show the
-- current Settings default, not whatever number happened to be seeded on it.
--
-- monthly_rent now reflects what is actually billed: the active contract's
-- rent when occupied, otherwise the Settings default. This is display-only --
-- generateInvoiceAction still reads contracts.monthly_rent directly, so this
-- change affects the floor plan / rooms list / reports, not billing math.
...
stable
as $$
  select coalesce((select value::text::numeric(12, 2) from settings where key = 'default_monthly_rent'), 0);
$$;

comment on function default_monthly_rent is
  'Settings default_monthly_rent, shown as a vacant room''s rent. Default 0.';
