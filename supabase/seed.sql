...
insert into settings (key, value, description) values
  ...
  ('payment_grace_days',    '0'::jsonb,    'Days past the due day before an invoice counts as overdue'),
  ('default_monthly_rent',  '3500'::jsonb, 'Move-in rent prefill when a room has no rent of its own set'),
  ('default_deposit',       '3500'::jsonb, 'Move-in deposit prefill when a room has no deposit of its own set'),
